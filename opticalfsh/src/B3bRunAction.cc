...
{  
  ...
  accumulableManager->RegisterAccumulable(fCerenkovCounts);
}
...
void B3bRunAction::EndOfRunAction(const G4Run* run)
{
  ...
  //results
  G4double scatter_fraction = 0.0, sensitivity = 0.0;
  if(fGoodEvents.GetValue() > 0){
	sensitivity = (fGoodEvents.GetValue()+fScatterEvents.GetValue())/nofEvents;
  ...
  G4cout
	 ...
	 << " Cerenkov Photons: " << fCerenkovCounts.GetValue() << G4endl
     << G4endl;

  G4double counts = 	 fGoodEvents.GetValue()+fScatterEvents.GetValue();
  //writing specific results to output file
  std::ofstream outfile;
  outfile.open("output.log", std::ios::app);
  outfile << nofEvents << " " << counts << " " << fScatterEvents.GetValue() << std::endl;
  outfile.close();
  auto analysisManager = G4AnalysisManager::Instance();
