...
{  
  ...
  G4Material* pg = nist->FindOrBuildMaterial("G4_PLEXIGLASS");
  ...
  fLogicCyl = new G4LogicalVolume(solidTank, water, "tankLV");
  ...
  G4double well_radius_outer = (0.3*fCylRadius)*mm;
  G4double well_length = 42*mm;
  G4RotationMatrix* well_rotate = new G4RotationMatrix();
  well_rotate->rotateZ(180*deg);
  
  
  G4ThreeVector well_position = G4ThreeVector(0, -tank_radius_outer+(0.1*tank_radius_outer)*mm, 0);
  G4Tubs* solidWell = new G4Tubs("well", well_radius_inner, well_radius_outer, 0.5*well_length, 0., M_PI);
  G4LogicalVolume* logicWell = new G4LogicalVolume(solidWell, water, "wellLV");
  new G4PVPlacement(well_rotate, well_position, logicWell, "well", fLogicWorld, false, 0, fCheckOverlaps);
...
void B3DetectorConstruction::ConstructSDandField()
{
  ...
  if(!fTank){
	...
	SetSensitiveDetector("wellLV", fTank);
  }
}
