...
{  
  ...
  //------------------------------------------------------------------------------------
  //------------------------------------------------------------------------------------
  //TODO: Redefine parameters to match fish
  G4Material* patient_mat = nist->FindOrBuildMaterial("G4_WATER");
  
  // Phantom Container
  //------------------------------------------------------------------------------------
  //------------------------------------------------------------------------------------
  //TO DO: Join a well to the cylinder tank. Fill it with water with a plastic casing.
  ...
  new G4PVPlacement(tank_rotate, G4ThreeVector(), logicTank, "Tank", logicWorld, false, 0, fCheckOverlaps);
  
  G4double well_radius_inner = 0*mm;
  G4double well_radius_outer = 5*mm;
  G4double well_length = 42*mm;
  G4RotationMatrix* well_rotate = new G4RotationMatrix();
  well_rotate->rotateZ(180*deg);
  
  G4ThreeVector well_position = G4ThreeVector(0, -tank_radius_outer+1*mm, 0);
  G4Tubs* solidWell = new G4Tubs("well", well_radius_inner, well_radius_outer, 0.5*well_length, 0., M_PI);
  ...
  //join the tank and well
  /*
  G4UnionSolid* fulltank = new G4UnionSolid("tank+well", solidTank, solidWell); 
  G4LogicalVolume* logicFullTank = new G4LogicalVolume(fulltank, patient_mat, "fullTankLV");
  new G4PVPlacement(0, G4ThreeVector(), logicFullTank, "tank+well", logicWorld, false, 0, fCheckOverlaps);
  */
