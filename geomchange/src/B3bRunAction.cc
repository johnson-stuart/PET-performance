...
void B3bRunAction::EndOfRunAction(const G4Run* run)
{
  ...
  const B3DetectorConstruction* detectorino = static_cast<const B3DetectorConstruction*>
  (G4RunManager::GetRunManager()->GetUserDetectorConstruction());
  
  G4double cylinder_r = detectorino->GetCylinderRadius();
  ...
  G4cout
     << "; Cylinder Radius: " << cylinder_r << G4endl
  ...
  outfile << scatter_fraction << " " << cylinder_r << std::endl;
  outfile.close();
  auto analysisManager = G4AnalysisManager::Instance();
